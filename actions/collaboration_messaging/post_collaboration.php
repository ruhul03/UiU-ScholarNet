...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    foreach ($columnsToEnsure as $columnName => $alterSql) {
        $check = db_query("SHOW COLUMNS FROM collaboration_posts LIKE ?", [$columnName], "s");
    ...
    // Insert new collaboration post
    $insert = db_query(
        "INSERT INTO collaboration_posts (user_id, title, department, description, skills_required, opportunity_type, status, slots_total, project_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
        [$user_id, $title, $department, $description, $skills, $opportunity_type, $status, $slots_total, $project_id],
        "issssssii"
    );

    if ($insert) {
        // REPUTATION POINTS: Award +20 points for posting a collaboration invitation
        db_query("UPDATE users SET points = points + 20 WHERE id = ?", [$user_id], "i");
