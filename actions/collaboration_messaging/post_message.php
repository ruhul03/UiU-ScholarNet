...
if ($receiver_id && $receiver_id > 0) {
    // DIRECT MESSAGE
    // Insert direct message into database using db_query helper
    db_query(
        "INSERT INTO messages (sender_id, receiver_id, channel, message, file_path, file_name) VALUES (?, ?, 'dm', ?, ?, ?)", 
    ...
    // CHANNEL CHAT
    // Insert channel chat message into database using db_query helper
    db_query(
        "INSERT INTO messages (sender_id, receiver_id, channel, message, file_path, file_name) VALUES (?, NULL, ?, ?, ?, ?)", 
        [$sender_id, $channel, $message, $file_path, $file_name], 
        "issss"
    );
    $message_id = $conn->insert_id;
